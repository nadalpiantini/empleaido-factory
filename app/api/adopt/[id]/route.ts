...
export async function POST(
  ...
  try {
    ...
    const empleaido = catalogData.find((e: Record<string, unknown>) => e.id === empleaidoId);
...
`;
    ...
    config.agents.list = config.agents.list.filter((a: Record<string, unknown>) => a.id !== agentId);
